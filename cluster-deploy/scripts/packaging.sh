...
config_init(){
  if [[  $1 == "old"  ]]; then
      egg_version=$(grep -E -m 1 -o "<eggroll.version>(.*)</eggroll.version>" ${source_code_dir}/eggroll_old/pom.xml| tr -d '[\\-a-z<>//]' | awk -F "eggroll.version" '{print $2}')
      meta_service_version=$(grep -E -m 1 -o "<eggroll.version>(.*)</eggroll.version>" ${source_code_dir}/eggroll_old/pom.xml| tr -d '[\\-a-z<>//]' | awk -F "eggroll.version" '{print $2}')
      roll_version=$(grep -E -m 1 -o "<eggroll.version>(.*)</eggroll.version>" ${source_code_dir}/eggroll_old/pom.xml| tr -d '[\\-a-z<>//]' | awk -F "eggroll.version" '{print $2}')
...
eggroll_old_compile(){
  eggroll_source_code_dir=${source_code_dir}/eggroll_old
cd ${eggroll_source_code_dir}
echo "[INFO] Compiling eggroll_old start"
mvn clean package -DskipTests
echo "[INFO] Compile eggroll_old done"

echo "[INFO] Moving eggroll_old start"
...
echo "[INFO] Moving eggroll_old done"
...
fate_compile(){
  ...
  if [ $1 == "old" ]; then
    cd ${source_code_dir}
    mv eggroll_old eggroll
    cd ${source_code_dir}/arch/
    mvn clean package -DskipTests
    cd ${source_code_dir}
    mv eggroll eggroll_old
  fi
  echo "[INFO] Compile fate done"
}
eggroll_old_package(){
  rm -rf ${packages_dir}/eggroll_old
  echo "[INFO] Package egg start"
  mkdir -p ${packages_dir}/eggroll_old/egg
  cd ${packages_dir}/eggroll_old/egg
  ...
  cp ${source_code_dir}/eggroll_old/framework/egg/src/main/resources/processor-starter.sh ./
  mkdir conf
  cp  ${source_code_dir}/eggroll_old/framework/egg/src/main/resources/egg.properties ./conf
  cp  ${source_code_dir}/eggroll_old/framework/egg/src/main/resources/log4j2.properties ./conf
  cp  ${source_code_dir}/eggroll_old/framework/egg/src/main/resources/applicationContext-egg.xml ./conf
  echo "[INFO] Package egg done"

  echo "[INFO] Package storage-service-cxx start"
  mkdir -p  ${packages_dir}/eggroll_old/storage-service-cxx
  cd ${packages_dir}/eggroll_old/storage-service-cxx
  ...
  mkdir -p ${packages_dir}/eggroll_old/python/eggroll/computing
  cd ${packages_dir}/eggroll_old/python/eggroll/computing
  ...
  mkdir -p ${packages_dir}/eggroll_old/python/eggroll/api
  cd ${packages_dir}/eggroll_old/python/eggroll/api
  mv ${packages_dir}/eggroll-api-${version}.tar.gz ./
  tar xzf eggroll-api-${version}.tar.gz
  rm -rf eggroll-api-${version}.tar.gz

  mkdir ${packages_dir}/eggroll_old/python/eggroll/conf
  cd  ${packages_dir}/eggroll_old/python/eggroll/conf
  ...
  mkdir -p ${packages_dir}/eggroll_old/meta-service
  cd  ${packages_dir}/eggroll_old/meta-service
  ...
  mkdir conf
  cp  ${source_code_dir}/eggroll_old/framework/meta-service/src/main/resources/meta-service.properties ./conf
  cp  ${source_code_dir}/eggroll_old/framework/meta-service/src/main/resources/log4j2.properties ./conf
  cp  ${source_code_dir}/eggroll_old/framework/meta-service/src/main/resources/applicationContext-meta-service.xml ./conf
  echo "[INFO] Package meta-service done"

  echo "[INFO] Package roll start"
  mkdir -p ${packages_dir}/eggroll_old/roll
  cd  ${packages_dir}/eggroll_old/roll
  ...
  mkdir conf
  cp  ${source_code_dir}/eggroll_old/framework/roll/src/main/resources/roll.properties ./conf
  cp  ${source_code_dir}/eggroll_old/framework/roll/src/main/resources/log4j2.properties ./conf
  cp  ${source_code_dir}/eggroll_old/framework/roll/src/main/resources/applicationContext-roll.xml ./conf
  echo "[INFO] Package roll done"
}

eggroll_new_package(){
  rm -rf ${packages_dir}/eggroll_new
  cd ${source_code_dir}/eggroll_new
  echo "[INFO] Package eggroll start"
  sh deploy/auto-packaging.sh
  mkdir -p ${packages_dir}/eggroll_new
  mv eggroll.tar.gz ${packages_dir}/eggroll_new
  cd ${packages_dir}/eggroll_new
...
arch_package(){
  ...
  mkdir conf
  cp ${cwd}/deploy/federatedml/generate_server_conf.py ${cwd}/deploy/federatedml/services.env ./conf
...
mysql_package(){
  ...
  if [ $1 == "old" ]; then
    ...
    cp ${source_code_dir}/eggroll_old/framework/meta-service/src/main/resources/create-meta-service.sql ./
  fi
...
global_env_package(){
  ...
  cp ./fate_base/env.sh ${packages_dir}/global_env
  echo "[INFO] Packaging arch done"
}
...
case "$1" in
    ...
    old)
      ...
      ;;
    *)
      echo "This type doesn't support!"
      exit
      ;;
esac
